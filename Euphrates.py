...
import argparse
parser = argparse.ArgumentParser(description="Euphrates Method")
...
visualDir = "EuphratesOut/base%s_visu/1I%dP" % (args.model, PredictWindow)
outputDir = "EuphratesOut/base%s/1I%dP/" % (args.model, PredictWindow)
# exit(1)
...
for idx ,video in tqdm(enumerate(videoList)):
    ...
    def mapBBox(frameId):
        ...
        BBoxlen = len(BBoxList[frameId])
        sumMVList = [None] * BBoxlen
        mvCountList = [0] * BBoxlen
        for idx in range(BBoxlen):
            sumMVList[idx] = torch.zeros(2)
        ...
        for dstFrameId, srcFrameId, dstX, dstY, srcX, srcY, width, height in MVlist[frameId]:
            for bboxId, bbox in enumerate(BBoxList[frameId].bbox):
                if bbox[0] - width <= srcX <= bbox[2] and bbox[1] - height <= srcY <= bbox[3]:
                    sumMVList[bboxId] += torch.Tensor([dstX-srcX, dstY-srcY])
                    mvCountList[bboxId] += 1
    
        for bboxId in range(BBoxlen):

            BBoxList[dstFrameId].bbox[bboxId] = movBBox(BBoxList[dstFrameId].bbox[bboxId],\
                sumMVList[bboxId][0]/(1e-6+mvCountList[bboxId]) , sumMVList[bboxId][1]/(1e-6+mvCountList[bboxId]), 0, 0)
...
torch.save(allVideoBBox, join(outputDir, "Euphrates1I%dP.pth" % PredictWindow))
